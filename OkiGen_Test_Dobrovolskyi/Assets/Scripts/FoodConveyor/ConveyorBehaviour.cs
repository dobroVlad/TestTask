...
public class ConveyorBehaviour : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        foreach (var food in _activeFood)
        {
            ...
            food.Body.velocity = new Vector3(direction.x, food.Body.velocity.y, direction.z) * _conveyorSpeed;
        }
    }
    ...
    void Update()
    {
        if (_started)
        {
            _myLineMaterial.mainTextureOffset += Vector2.up * _conveyorSpeed * Time.deltaTime;
        }
    }
